...
Descripción | Param. #1 | Param. #2 | Param. #3 | Valor de Retorno
:-----------: | :-----------: | :-----------: | :-----------: | :-----------:
**Compara** dos cadenas (s1 y s2) carácter por carácter, hasta un máximo de n caracteres. S| Primera cadena a comparar | Segunda cadena a comparar | Número máximo de caracteres a comparar | Si las cadenas son idénticas en esos primeros `n` caracteres, devuelve **0**. si el caracter encontrado difiere : Negativo `s1` < `s2` y positivo si `s1` > `s2`. Es útil para comparar prefijos de cadenas. .
